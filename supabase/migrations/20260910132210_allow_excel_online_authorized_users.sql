...
begin
  ...
  if existing_id is null then
    perform vault.create_secret(p_value, p_name, 'Configuração privada da sincronização do Excel Online');
  else
    perform vault.update_secret(existing_id, p_value, p_name, 'Configuração privada da sincronização do Excel Online');
  end if;
  return true;
end;
$$;
