...
class EstoqueApp(ctk.CTk):
    ...
    def count_page(self):
        ...
        listing=Card(body);listing.grid(row=0,column=1,sticky="nsew");bar=ctk.CTkFrame(listing,fg_color="transparent");bar.pack(fill="x",padx=20,pady=(14,10));ctk.CTkLabel(bar,text="Check-in dos produtos",text_color=COLORS["text"],font=ctk.CTkFont("Inter",16,"bold")).pack(anchor="w")
        filters=ctk.CTkFrame(bar,fg_color="transparent");filters.pack(fill="x",pady=(10,0))
        self.count_filter=tk.StringVar(value=self.settings.get("count_filter","todos"));ctk.CTkOptionMenu(filters,variable=self.count_filter,values=["todos","pendentes","verificados"],width=110,height=36,fg_color=COLORS["surface_alt"],button_color=COLORS["surface_hover"],text_color=COLORS["text"],command=lambda _value:(self.refresh_counts(),self.save_interface_state())).pack(side="right")
        ctk.CTkButton(filters,text="Imprimir estoque",image=self.icons["print"],width=135,height=36,corner_radius=9,fg_color=COLORS["surface_alt"],hover_color=COLORS["surface_hover"],text_color=COLORS["text"],command=self.print_current_stock).pack(side="right",padx=(0,8))
        self.count_search=ctk.CTkEntry(filters,placeholder_text="Filtrar produto...",height=36,corner_radius=9);self.count_search.pack(side="left",fill="x",expand=True,padx=(0,8));self.count_search.insert(0,self.settings.get("count_search",""));self.count_search.bind("<KeyRelease>",lambda _event:self.schedule_ui_task("count_table_search",lambda:(self.refresh_counts(),self.save_interface_state()),100))
        actions=ctk.CTkFrame(bar,fg_color="transparent");actions.pack(fill="x",pady=(8,0))
        ctk.CTkButton(actions,text="Contar",image=self.icons["count"],width=95,height=36,corner_radius=9,fg_color=COLORS["surface_alt"],hover_color=COLORS["surface_hover"],text_color=COLORS["text"],command=self.prepare_count).pack(side="right")
        ctk.CTkButton(actions,text="Editar produto",image=self.icons["edit"],width=125,height=36,corner_radius=9,fg_color=COLORS["surface_alt"],hover_color=COLORS["surface_hover"],text_color=COLORS["text"],command=self.edit_selected_count_product).pack(side="right",padx=(0,8))
        ctk.CTkButton(actions,text="Explicar",width=90,height=36,corner_radius=9,fg_color=COLORS["surface_alt"],hover_color=COLORS["surface_hover"],text_color=COLORS["text"],command=self.explain_confidence).pack(side="right",padx=(0,8))
